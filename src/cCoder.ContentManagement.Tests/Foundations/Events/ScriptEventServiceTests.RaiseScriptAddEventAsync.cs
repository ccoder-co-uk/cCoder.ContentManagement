...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseScriptAddEventAsync()
    {
        // Given
        Script entity = new();
        ...
        scriptEventBrokerMock
            .Setup(expression: x => x.RaiseScriptAddEventAsync(message: It.IsAny<EventMessage<CmsDataModels.Script>>()))
...
expression: x => x.RaiseScriptAddEventAsync(message: It.IsAny<EventMessage<CmsDataModels.Script>>()),
times: Times.Once
        );
