...
public partial class AppEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseAppUpdateEventAsync()
    {
        ...
        appEventBrokerMock
            .Setup(x => x.RaiseAppUpdateEventAsync(It.IsAny<EventMessage<CmsDataModels.App>>()))
            .Callback<EventMessage<CmsDataModels.App>>(message => actualMessage = message)
            .Returns(ValueTask.CompletedTask);

        // When
        await service.RaiseAppUpdateEventAsync(entity);
        ...
        appEventBrokerMock.Verify(
            x => x.RaiseAppUpdateEventAsync(It.IsAny<EventMessage<CmsDataModels.App>>()),
            Times.Once
        );
