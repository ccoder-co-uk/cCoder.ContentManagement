...
public partial class ResourceEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseResourceAddEventAsync()
    {
        ...
        resourceEventBrokerMock
            .Setup(x => x.RaiseResourceAddEventAsync(It.IsAny<EventMessage<CmsDataModels.Resource>>()))
            .Callback<EventMessage<CmsDataModels.Resource>>(message => actualMessage = message)
            .Returns(ValueTask.CompletedTask);

        // When
        await service.RaiseResourceAddEventAsync(entity);
        ...
        resourceEventBrokerMock.Verify(
            x => x.RaiseResourceAddEventAsync(It.IsAny<EventMessage<CmsDataModels.Resource>>()),
            Times.Once
        );
