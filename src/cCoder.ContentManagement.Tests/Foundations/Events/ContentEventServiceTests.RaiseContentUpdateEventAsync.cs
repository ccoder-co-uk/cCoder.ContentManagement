...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseContentUpdateEventAsync()
    {
        // Given
        Content entity = new();
        ...
        contentEventBrokerMock
            .Setup(x => x.RaiseContentUpdateEventAsync(It.IsAny<EventMessage<CmsDataModels.Content>>()))
        ...
        contentEventBrokerMock.Verify(
            x => x.RaiseContentUpdateEventAsync(It.IsAny<EventMessage<CmsDataModels.Content>>()),
            Times.Once
        );
