...
public partial class LayoutEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseLayoutAddEventAsync()
    {
        ...
        layoutEventBrokerMock
            .Setup(expression: x => x.RaiseLayoutAddEventAsync(message: It.IsAny<EventMessage<CmsDataModels.Layout>>()))
            .Callback<EventMessage<CmsDataModels.Layout>>(action: message => actualMessage = message)
            .Returns(value: ValueTask.CompletedTask);

        // When
        await service.RaiseLayoutAddEventAsync(
            layout: entity,
...
expression: x => x.RaiseLayoutAddEventAsync(message: It.IsAny<EventMessage<CmsDataModels.Layout>>()),
times: Times.Once
        );
