...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseComponentAddEventAsync()
    {
        // Given
        ...
        componentEventBrokerMock
            .Setup(expression: x => x.RaiseComponentAddEventAsync(message: It.IsAny<EventMessage<CmsDataModels.Component>>()))
            .Callback<EventMessage<CmsDataModels.Component>>(action: message => actualMessage = message)
            .Returns(value: ValueTask.CompletedTask);

        // When
        await service.RaiseComponentAddEventAsync(component: entity);
...
expression: x => x.RaiseComponentAddEventAsync(message: It.IsAny<EventMessage<CmsDataModels.Component>>()),
times: Times.Once
        );
