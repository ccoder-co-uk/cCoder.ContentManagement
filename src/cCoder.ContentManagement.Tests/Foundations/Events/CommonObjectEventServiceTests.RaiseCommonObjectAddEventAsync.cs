...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseCommonObjectAddEventAsync()
    {
        // Given
        ...
        commonObjectEventBrokerMock
            .Setup(expression: x => x.RaiseCommonObjectAddEventAsync(message: It.IsAny<EventMessage<CommonObject>>()))
...
expression: x => x.RaiseCommonObjectAddEventAsync(message: It.IsAny<EventMessage<CommonObject>>()),
times: Times.Once
        );
