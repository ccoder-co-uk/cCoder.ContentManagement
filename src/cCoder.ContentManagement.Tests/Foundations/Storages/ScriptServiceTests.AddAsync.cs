...
public partial class ScriptServiceTests
{
    ...
    public async Task ShouldDelegateToBrokerWhenUserIsAuthorizedForAddAsync()
    {
        ...
        Script script = CreateRandomScript(appId: 7);

        CmsDataModels.Script submitted = null;


        authorizationBrokerMock.Setup(x => x.Authorize((int?)7, "Script_create"));

        scriptBrokerMock
            .Setup(x =>
                x.AddScriptAsync(It.Is<CmsDataModels.Script>(candidate => !ReferenceEquals(candidate, script)))
            )
            .Callback<CmsDataModels.Script>(candidate => submitted = candidate)
            .ReturnsAsync((CmsDataModels.Script value) => value);

        // When
        Script result = await scriptService.AddScriptAsync(script);
        ...
        scriptBrokerMock.Verify(
            x => x.AddScriptAsync(It.Is<CmsDataModels.Script>(candidate => !ReferenceEquals(candidate, script))),
            Times.Once
        );
        scriptBrokerMock.VerifyNoOtherCalls();
        authorizationBrokerMock.Verify(x => x.Authorize((int?)7, "Script_create"), Times.Once);
    }
    ...
    public async Task ShouldThrowSecurityExceptionWhenUserLacksCreatePrivilegeForAddAsync()
    {
        ...
        authorizationBrokerMock
            .Setup(x => x.Authorize((int?)7, "Script_create"))
            .Throws(new SecurityException("Access Denied!"));

        // When
        Func<Task> action = async () => await scriptService.AddScriptAsync(script);
