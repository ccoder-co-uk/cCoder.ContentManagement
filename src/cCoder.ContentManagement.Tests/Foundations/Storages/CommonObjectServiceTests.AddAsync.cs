...
public partial class CommonObjectServiceTests
{
    ...
    public async Task ShouldDelegateToBrokerWhenUserIsAuthorizedForAddAsync()
    {
        ...
        authorizationBrokerMock.Setup(x => x.Authorize((int?)7, "CommonObject_create"));

        commonObjectBrokerMock
            .Setup(x =>
                x.AddCommonObjectAsync(
                    It.IsAny<DataCommonObject>()
                )
            )
        ...
        commonObjectBrokerMock.Verify(
            x =>
                x.AddCommonObjectAsync(
                    It.Is<DataCommonObject>(candidate => candidate.Id == commonObject.Id)
                ),
            Times.Once
        );
        commonObjectBrokerMock.Verify(
        ...
        authorizationBrokerMock.Verify(
            x => x.Authorize((int?)7, "CommonObject_create"),
            Times.Once
        );
    }
    ...
    public async Task ShouldThrowSecurityExceptionWhenUserLacksCreatePrivilegeForAddAsync()
    {
        ...
        authorizationBrokerMock
            .Setup(x => x.Authorize((int?)7, "CommonObject_create"))
