    ...
    IScriptService scriptService = null) : IComponentRenderProcessingService
{
    ...
    public string Render(int appId, string name, User user, string culture, string theme)
    {
        ...
        Component component = app?.Components
            .Where(existingComponent => existingComponent.AppId == appId)
            .FirstOrDefault(existingComponent =>
                existingComponent.Name.Equals(name, StringComparison.OrdinalIgnoreCase))
            ?? objectCache.Get<Component>("component|" + name.ToLower())
            ?? throw new InvalidOperationException("Component '" + name + "' was not found.");

        ComponentRenderParams renderParams = new(theme, app, user, culture);
        return RenderComponent(component, renderParams);
    }
    ...
    public string RenderComponent(Component component, ComponentRenderParams renderParams)
    {
        ...
        ValidateComponentRenderParams(renderParams, "renderParams");
        ICollection<Replacement> replacements = DefaultReplacements(renderParams);
        return $"<section name='{component.Name}' class='component' data-id='{component.Id}' data-resource-key='{component.ResourceKey}'>{ProcessContentString(component.ResourceKey, renderParams, component.Content, replacements)}<script type='text/javascript'>{ProcessContentString(component.ResourceKey, renderParams, component.Script, replacements)}</script></section>";
    }
    ...
    private ICollection<Replacement> DefaultReplacements(RenderParams renderParams)
    {
        ...
        {
            list2.Add(new Replacement("[theme[name]]", componentRenderParams.Theme));
    ...
    private void Dms(string key, StringBuilder source, ComponentRenderParams renderParams, IEnumerable<Replacement> replacements)
    {
        RegexReplace(source, "\\[dms\\[[A-Za-z\\d_/. \\-]*\\]\\]", delegate (Match match)
        {
            ...
            return string.IsNullOrEmpty(latestTextContent) ? string.Empty : ProcessContentString(key, renderParams, latestTextContent, replacements);
        });
    }
