window.ContentManagementApi = {
    tokenKey: "contentManagementAuthToken",
    token: null,
    currentUser: null,
    ...
    refreshAuthState: async function () {
        try {
            const user = await this.get("/Api/Account/Me");
    ...
    setAuthState: function (user) {
        ...
        document.dispatchEvent(new CustomEvent("content-management-auth-changed", {
    ...
    notify: function (message, isError) {
        ...
        status.classList.toggle("cm-status-error", Boolean(isError));
    }
};

document.addEventListener("DOMContentLoaded", () => window.ContentManagementApi.init());
